...
dotenv.config();
export const setUpContainer = async (id, type) => {
    ...
    await execAsync(`docker container run -d -p 0:${port} --name ${containerName} -v "${absolutePath}:${containerPath}" ${imageName}`);
...
export const execContainer = async (socket, containerId) => {
...
export const removeImage = async (id) => {
    ...
    const image = images.find((img) => img.RepoTags && img.RepoTags.includes(`${imageName}:latest`));
    if (!image) {
